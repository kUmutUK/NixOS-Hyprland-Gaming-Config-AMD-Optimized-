...
backup() {
  local src=$1
  if [ -e "$src" ]; then
    # Resolve symlink and copy actual content (Home Manager symlinks → nix store)
...
# FIX: Home Manager creates symlinks from ~/.config/* to the read-only Nix store.
#      We must remove them before copying, otherwise cp fails with "Read-only file system".
#      Backup was already taken in Step 2 with -L (follows symlinks → copies real content).
# =============================================================================
echo ""
...
copy_config() {
  ...
  if [ -d "$src" ]; then
    # Remove existing dir or symlink (may point to read-only nix store)
    if [ -e "$dst" ] || [ -L "$dst" ]; then
